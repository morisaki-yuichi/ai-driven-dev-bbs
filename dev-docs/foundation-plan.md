...
- **`migrations/*.sql` はスキーマ定義（DDL）のみとし、`INSERT` によるシードデータを含めない**（H-11）。テストユーザー・初期スレッド等の投入をマイグレーションに紛れ込ませると、H-08（空DB開始）とH-11（シード前提の禁止）を静かに破る。シナリオ内で作成されるデータ（`testuser_01` 等）だけが存在する状態を、マイグレーション適用直後のDBが保つこと。
...
| 9 | `templates/layout.html` | C-01 の固定文言のうち **ヘッダー／フッター／ユーザー表示の3つ**（`AI掲示板（仮）` / `© 2026 AI駆動開発教材プロジェクト` / `[表示名] さん`）を1箇所に固定 |
| 10 | `domain/` の定数（例: `model.rs` の `DELETED_COMMENT_TEXT`） | C-01 の固定文言のうち **残る1つ**（削除済みコメント本文 `＜このコメントは削除されました＞`。AC08-2 / AC10-3、[0016](decisions/0016-tech-stack.md) §4 が E4 の代表例として名指し）を1箇所に固定。ヘッダー／フッターに属さないため #9 には置けない。domain 層に置くことで、Askama のテンプレートレンダリングを介さない単体テストからも文字列一致を検証できる。テンプレート側はこの定数（またはこれを返すメソッド）を参照するだけにし、リテラルを書き写さない |
| 11 | `web/params.rs`（`q` / `sort` / `page` の共通パース） | [0011](decisions/0011-search-matching-and-normalization.md) §影響（全パラメータをクエリ文字列で保持）、C-13、[0013](decisions/0013-pagination-edge-cases.md)（不正値を1ページ目に丸める） |
| 12 | `domain/` の骨格（`validation.rs` / `query.rs`）とその単体テスト | Lean モデルの対応先。**F** の高速テスト基盤。**A** の取り返し論証の前提。#10 の定数もこの `domain/` 配下に置く |
| 13 | `tests/` の `#[sqlx::test]` ひな形 | **F**。テストごとの使い捨て DB |
| 14 | `SETUP.md` ＋ `README.md` へのリンク追記 | H-06 / P-08 / §4.3・§4.4 |
| 15 | **B 緩和策**: `mold` または `lld` リンカ指定、`[profile.dev] debug=1`、`cargo-watch`（または `bacon`）、`cargo check` を既定の高速パスにする。**リンカ指定は評価者経路を壊さない形にすること**（§6 の注記） | §6 |

**着手順の意図**: 1〜5 で「評価者が clone → `docker compose up` で起動できる」骨格を先に通す。**H-06 の充足が最も後戻りコストが高い**ため、機能実装より前に空アプリで一度通しておく。6〜11 が横断的制約（404・認証・固定文言・クエリパラメータ）を1箇所に固める層で、ここを先に作らないと各機能に散らばって C-01 / C-09 / C-10 / C-13 の漏れが生じる。12〜13 でテストの背骨を通してから機能実装へ入る。
...
**2026-07-19 — フェーズ2 独立レビュー（`dev-docs/reviews/phase2-review.md`）の上流エスカレーション反映。**

| 指摘 | 対応 | 内容 |
| :--- | :--- | :--- |
| 上流エスカレーション | 反映（ユーザー選択: 新規項目追加） | §5 に新規 #10（`domain/` の定数、例 `DELETED_COMMENT_TEXT`）を追加し、C-01 の固定文言4つのうち #9（`layout.html`）でカバーされていなかった削除済みコメント本文 `＜このコメントは削除されました＞` の集約先を割り当てた。以降の項目番号を1つずつ繰り下げ（旧10〜14 → 新11〜15）、「着手順の意図」の範囲表記（6〜10→6〜11、11〜12→12〜13）も合わせて更新した。
