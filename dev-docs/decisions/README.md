...
2026-07-19 時点で、0001〜0018 は `ai+user`（AI が提案し人間が採否を判断）だが、**0019 は AI が単独判断で先へ進めた未承認の決定**である。この索引は、そうした決定を洗い出すために置く。**ここに項目が増えた場合、それは「レビューされていない前提の上に実装が積まれている」ことを意味する**ので、フェーズの区切りで必ず棚卸しする。
