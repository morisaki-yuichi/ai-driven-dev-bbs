...
2026-07-19 時点で、0001〜0017 のすべてが `ai+user`（AI が提案し人間が採否を判断）である。この索引は、後続フェーズで AI が単独判断で先へ進めた決定を洗い出すために置く。**ここに項目が増えた場合、それは「レビューされていない前提の上に実装が積まれている」ことを意味する**ので、フェーズの区切りで必ず棚卸しする。
