...
2026-07-20 時点で、0001〜0030 のすべてが `ai+user`（AI が提案し人間が採否を判断）である。0019〜0022・0024〜0028 は起票時点では AI 単独判断（`decided_by: ai`）だったが、いずれもユーザーが承認し `ai+user` に更新した。0026・0027 はF09のレビューでの裁定に伴う承認で、このとき 0026 は言明の形そのものが (b)（`paginate` の丸め規約を右辺へ転記する）から (a)（仮定 `1 ≤ p` を置く）へ変更されている ―― 前者は `p = 0` の1点で不変条件が実装を追認するだけになる、という指摘による。**形式モデルの言明に関する決定は「実装を追認していないか」が承認時の主要な観点になる**ことを、後続セッションの参考として記しておく。0028 はF10のレビューでの裁定に伴う承認で、決定内容そのものに変更はない。この索引は、後続フェーズでAIが単独判断で先へ進めた決定を洗い出すために置く。**ここに項目が増えた場合、それは「レビューされていない前提の上に実装が積まれている」ことを意味する**ので、フェーズの区切りで必ず棚卸しする。
