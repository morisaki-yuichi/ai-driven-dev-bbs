...
| [0019](0019-schema-details.md) | D02: スキーマ詳細（主キー型・URLのID形式・インデックス） | 提案 | major | ai | D02 |
| [0020](0020-profile-edit-path.md) | D10: ユーザー編集画面（P06）のパス | 提案 | minor | ai | D10 |

### 索引: `decided_by = ai`（人間の承認を経ていない暫定決定）

| 番号 | タイトル | importance |
| :--- | :--- | :--- |
| [0019](0019-schema-details.md) | D02: スキーマ詳細（主キー型・URLのID形式・インデックス） | major |
| [0020](0020-profile-edit-path.md) | D10: ユーザー編集画面（P06）のパス | minor |

2026-07-19 時点で、0001〜0018 は `ai+user`（AI が提案し人間が採否を判断）だが、**0019・0020 は AI が単独判断で先へ進めた未承認の決定**である。この索引は、そうした決定を洗い出すために置く。**ここに項目が増えた場合、それは「レビューされていない前提の上に実装が積まれている」ことを意味する**ので、フェーズの区切りで必ず棚卸しする。
