...
| [0025](0025-lean-invariant-scope-per-feature-session.md) | 機能セッションごとのLean証明スコープ（横断的invariantは実装済み範囲に絞る） | 決定済 | minor | ai+user | 新規（F05形式化作業中に判明） |
| [0026](0026-hasnext-invariant-zero-page-fix.md) | hasNext_iff_more（Leanモデル）のp=0反例修正 | 決定済 | minor | ai+user | 新規（F09形式化作業中に判明） |
| [0027](0027-lastupdated-invariant-clock-monotonicity.md) | comment_bumps_lastUpdated（Leanモデル）に論理時計の単調性を仮定として追加する | 決定済 | minor | ai+user | 新規（F09実装差分のレビュー中に判明） |
| [0028](0028-error-page-auth-reflection.md) | エラーページへの認証状態の反映方式（マーカー＋ミドルウェア後処理） | 決定済 | minor | ai+user | 新規（F10実装差分のレビュー中に判明） |
| [0029](0029-created-at-clock-divergence.md) | 形式モデルの論理時計(tick)と実装のcreated_at(now())の乖離を許容する | 決定済 | minor | ai+user | 新規（F07実装差分のレビュー中に判明） |
| [0030](0030-comment-deletion-no-confirm-dialog.md) | コメント削除の確認方式（確認ダイアログなしで即削除） | 決定済 | minor | ai+user | D18（`dev-docs/requirements-analysis.md` §3） |
| [0031](0031-thread-deletion-row-locking.md) | F06スレッド削除における行ロック方針（F08のTOCTOU対策パターンを移植できない場合の代替） | 決定済 | major | ai+user | 新規（F06実装レビューで判明） |
| [0032](0032-like-wildcard-escaping.md) | F11検索でのLIKEワイルドカード文字（%・_）のエスケープ | 決定済 | minor | ai+user | 新規（F11実装中に判明） |
| [0033](0033-search-query-trim-and-max-length.md) | 検索語qのトリムと最大長 | 決定済 | minor | ai+user | D07（decision 0011の続き。レビューで判明） |
| [0034](0034-sort-ui-labels.md) | F12ソートUIの表示ラベル文言 | 決定済 | minor | ai+user | 新規（F12実装中に判明） |
| [0035](0035-display-name-uniqueness-not-required.md) | 表示名の一意性は要求しない（同名改名によるなりすまし表示を意図的に許容する） | 決定済 | minor | ai+user | D16（`dev-docs/requirements-analysis.md` §3） |
| [0036](0036-csrf-form-missing-fields-422.md) | 必須フィールド欠落POSTが403でなく422になる件を受容する | 決定済 | minor | ai+user | D05（`dev-docs/requirements-analysis.md` §3） |
| [0037](0037-display-name-no-maxlength.md) | 表示名入力欄から maxlength を外す(0023の観測性方針をnovalidate非対象の属性へ拡張) | 決定済 | minor | ai+user | 新規（最終ゲート・評価者経路の通し確認で判明） |

### 索引: `decided_by = ai`（人間の承認を経ていない暫定決定）

**該当なし（0件）。**

2026-07-21 時点で、0001〜0037 のすべてが `ai+user`（AI が提案し人間が採否を判断）である。0019〜0022・0024〜0028 は起票時点では AI 単独判断（`decided_by: ai`）だったが、いずれもユーザーが承認し `ai+user` に更新した。0026・0027 はF09のレビューでの裁定に伴う承認で、このとき 0026 は言明の形そのものが (b)（`paginate` の丸め規約を右辺へ転記する）から (a)（仮定 `1 ≤ p` を置く）へ変更されている ―― 前者は `p = 0` の1点で不変条件が実装を追認するだけになる、という指摘による。**形式モデルの言明に関する決定は「実装を追認していないか」が承認時の主要な観点になる**ことを、後続セッションの参考として記しておく。0028 はF10のレビューでの裁定に伴う承認で、決定内容そのものに変更はない。0031 はF06のレビューでの裁定に伴う承認で、行ロック方針は既に実装済み（`app/src/db/threads.rs`）であり、この決定はその方針を正式に記録するもの。0034 はF12実装セッションでAIが単独判断のまま起票した（ソートラベルの日本語文言）が、F12のレビュー裁定でユーザーが承認し `ai+user` に更新した。このとき、JSなし（decision 0008）で `<select>` の変更が送信ボタン押下でしか反映されない点が `agent-browser`（H-12）の観測性・操作性のリスクとして指摘され、**送信ボタンのラベルを `検索` から `検索・並び替え` へ変更する決定が同じ 0034 に追加**されている。0035 はF04プロフィール編集のレビューでユーザーが起点となって記録を指示した決定で、AI単独判断の期間を経ずに `ai+user` として起票している。0036 は持ち越し事項の専用サイクルで、F05から6回連続で持ち越されていた「必須フィールド欠落POSTが403でなく422になる」件についてAIが調査・選択肢整理を行い、ユーザーが受容の可否を裁定したもの（コード変更なし）で、AI単独判断の期間を経ずに `ai+user` として起票している。0037 は全12機能実装後の最終ゲート（評価者経路の通し確認）で判明した観測性の問題（表示名欄の `maxlength` に阻まれ `agent-browser` が16文字目を入力できず、AC01-4・シナリオ05-1-5のエラー表示を観測できない）についてユーザーが裁定した修正で、AI単独判断の期間を経ずに `ai+user` として起票している。この索引は、後続フェーズでAIが単独判断で先へ進めた決定を洗い出すために置く。**ここに項目が増えた場合、それは「レビューされていない前提の上に実装が積まれている」ことを意味する**ので、フェーズの区切りで必ず棚卸しする。
