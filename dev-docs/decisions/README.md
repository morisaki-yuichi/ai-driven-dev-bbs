...
2026-07-19 時点で、0001〜0020 のすべてが `ai+user`（AI が提案し人間が採否を判断）である。0019・0020 は起票時点では AI 単独判断（`decided_by: ai`）だったが、同日中にユーザーが承認し `ai+user` に更新した。この索引は、後続フェーズで AI が単独判断で先へ進めた決定を洗い出すために置く。**ここに項目が増えた場合、それは「レビューされていない前提の上に実装が積まれている」ことを意味する**ので、フェーズの区切りで必ず棚卸しする。
